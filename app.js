...
const EN = {
  ui: {
    ...
    foot: [
      ...
      '<strong>Alignment</strong> picks the key and the mode — and, more importantly, the one degree that mode bends. A theme that never sounds its mode’s <strong>colour note</strong> is in that mode on paper only, so every theme lands on it once a phrase. Plain major bends nothing, which is why it gets a suspended chord and a pedal bass instead of a colour.',
      'A race does not sing all seven of those degrees. A dwarf sings five, a dragonborn walks past the third and the seventh, a human leaves nothing out — <strong>the gaps in the scale</strong> are audible in the first phrase and never argue with the mode, because any key can be pentatonic. The harmony underneath keeps all seven, which is how gapped scales are played everywhere they are actually played. The tonic, the fifth and the colour note are never taken away.',
      'A <strong>second class</strong> answers the motif in another voice, speaking in the melody’s silences rather than beside it, and the race brings <strong>an instrument of its own</strong> — a harp for elves, an organ for dwarves — that arrives partway through and leaves again, plus <strong>its own kit</strong>: a drum is what a people plays, not what a profession plays.',
